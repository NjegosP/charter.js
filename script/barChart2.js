var Chart = function (input) {
	var context,
		...
		render = function (input) {
			...
			clickHandlerArray = new Array(inputObject.dataY.length).fill(null);
			hoverHandlerArray = new Array(inputObject.dataY.length).fill(null);
			bottomStartingPoint = populateArray();
			originalCoordinates = Array.from(coordinates);
			doCalculation();
			drawChartParts(input);
			animateBars();

			canvas.addEventListener("mousemove", hoverOverBar, false);
			canvas.addEventListener("mousemove", cursorChange, false);
			canvas.addEventListener("mousemove", hoverOverSection, false);
			//canvas.addEventListener("mousemove", hoverOverLegend, false);
			canvas.addEventListener("click", mouseClickOnLegend, false);
		},
		...
		animateBars = function () {
			var myReq = window.requestAnimationFrame(animateBars);
			var startingPoints = getSectionStartingPoint(); // get starting point of each section in the chart
			var numberOfGaps = numberOfBars - 1; // gaps between separate bars
			var barWidth = inputObject.dataPointSize; // each bar width
			var innerMargin = 5; // left and right margin of each section
			var maxBarWidth =
				(sectionWidth - innerMargin * numberOfGaps) / numberOfBars - 2; // calculate the maximum width that can fit the section
			...
			var margin =
				...
				2;
			var max = getMaxCoordinate(maxY);

			for (var i = 0; i < numberOfBars; i++) {
				if (bottomStartingPoint[i] !== null) {
			...
			for (var i = 0; i < numberOfBars; i++) {
				if (bottomStartingPoint[i] !== null) {
					for (var k = 0; k < numberOfSections; k++) {
						if (bottomStartingPoint[i][k] !== null) {
							if (
								bottomStartingPoint[i][k] >=
								canvasHeight - max - bottomMargin
							) {
								window.cancelAnimationFrame(myReq);
								for (var i = 0; i < bottomStartingPoint.length; i++) {
									bottomStartingPoint[i].fill(0);
								}
								clearChart();
								drawChartParts();
								drawBars();
		...
		populateArray = function () {
			var arr = new Array(inputObject.dataYNames.length).fill(0);
			var nonNullArray = inputObject.dataY.filter((el) => {
				if (el !== null) {
					return el;
				}
			});
			for (var i = 0; i < arr.length; i++) {
				arr[i] = new Array(nonNullArray[0].length).fill(0);
			}
			return arr;
		},
		hoverOverSection = function () {
			...
			var startingPointsX = getSectionStartingPoint();
			startingPointsX.push(
				startingPointsX[startingPointsX.length - 1] + sectionWidth
			);
			var startPoint = margin.top + chartHeight;
			var endPoint = margin.top;

			for (let i = 0; i < numberOfSections; i++) {
				let sectionCenter = startingPointsX[i] + sectionWidth / 2;
				if (
					relativeX > startingPointsX[i] &&
					relativeX < startingPointsX[i + 1] &&
					relativeY > endPoint &&
					relativeY < startPoint &&
					isCursorOverSection === null
				) {
					drawRectangle(
						startingPointsX[i],
					...
					isCursorOverSection = sectionCenter;
				} else if (
					(relativeX < startingPointsX[i] ||
						relativeX > startingPointsX[i + 1] ||
						relativeY < endPoint ||
		...
		hoverOverLegend = function (event) {
			var r = canvas.getBoundingClientRect();
			var topLeftCornerPosition = {x: r.x, y: r.y};
			var posX = event.clientX; // Get the horizontal coordinate
			var posY = event.clientY; // Get the vertical coordinate
			...
			for (var i = 0; i < inputObject.dataYNames.length; i++) {
				if (
					...
					relativeY < currentYCoordinate + 10 &&
					hoverHandlerArray[i] == null
				) {
					if (inputObject.dataY[i] !== null) {
						var nonNullArrays = inputObject.dataY.filter((el) => {
							if (el !== null) {
								return el;
							}
						});
						inputObject.colors.fill("rgba(255, 255, 255, 0.2)");
						inputObject.colors[i] = originalColors[i];
						clearChart();
						drawChartParts();
						drawBars();
						hoverHandlerArray[i] = true;
					}
				} else if (
					(relativeX < currentXCoordinates[i] ||
						relativeX > currentXCoordinates[i + 1] + 1 ||
						relativeY < currentYCoordinate - 10 ||
						relativeY > currentYCoordinate + 10) &&
					hoverHandlerArray[i] == true
				) {
					...
					const isNull = hoverHandlerArray.every((item) => !item);
					if (isNull) {
						for (var i = 0; i < inputObject.colors.length + 1; i++) {
							if (
								inputObject.colors[i] === "rgba(255, 255, 255, 0.2)" &&
								inputObject.dataY[i] !== null
							) {
								inputObject.colors[i] = originalColors[i];
							} else if (
								inputObject.colors[i] === "rgba(255, 255, 255, 0.2)" &&
					...
					//inputObject.colors = [...originalColors];

					// hoverHandlerArray[i] = null;
				}
			}
		},
		// drawSectionSeparators = function () {
		// 	var startingPointsX = getSectionStartingPoint();
		// 	var startPoint = margin.top + chartHeight;
		// 	var endPoint = margin.top;

		// 	for (let i = 1; i < numberOfSections; i++) {
		// 		drawLine(startingPointsX[i], startPoint, endPoint);
		// 	}
		// },
		// drawLine = function (x, startingPointY, endPointY) {
		// 	context.strokeStyle = inputObject.layoutColor;
		// 	context.lineWidth = 0.2;
		// 	context.beginPath();
		// 	context.moveTo(x, startingPointY);
		// 	context.lineTo(x, endPointY);
		// 	context.stroke();
		// },
		...
		mouseClickOnLegend = function (event) {
			var r = canvas.getBoundingClientRect();
			var topLeftCornerPosition = {x: r.x, y: r.y};
			var posX = event.clientX; // Get the horizontal coordinate
			var posY = event.clientY; // Get the vertical coordinate
			...
			console.log(inputObject.colors);
			console.log(inputObject.dataY);
			for (var i = 0; i < inputObject.dataYNames.length; i++) {
				if (
					relativeX > currentXCoordinates[i] &&
					relativeX < currentXCoordinates[i + 1] &&
					...
					for (var k = 0; k < numberOfSections; k++) {
						if (coordinates[k] !== null) {
							...
							context.fillRect(
								...
								chartTopLeftCorner.y + chartHeight,
								barWidth,
								-(canvasHeight - coordinates[i][k].y - 90)
							);
						}
					}
				}
		...
		getMaxCoordinate = function (max) {
			var posY;

			if (max !== null) {
				var percentage = ((max - minY) * percentTaken) / maxMinDiff;
				posY =
					chartHeight -
					(chartHeight / 100) * percentFromZero -
					(chartHeight / 100) * percentage;
			}
			return posY + margin.top;
		},
...
exampleData2 = {
	...
	dataY: [
		...
		[321, 555, 221, 145, 432, 200, 100, 150, 250, 333, 129, 145],
